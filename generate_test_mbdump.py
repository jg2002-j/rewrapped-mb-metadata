...
def reader_for(member_file):
    # Stream text lines and split manually to bypass csv parser limits
    for line in io.TextIOWrapper(member_file, encoding="utf-8", errors="ignore"):
        yield line.rstrip('\r\n').split('\t')
...
def build_test_dump(source_tar, output_tar):
    ...
    print(f"Selected {len(release_ids)} releases across {len(release_group_ids)} release groups")
...
if __name__ == "__main__":
    build_test_dump("mbdump.tar.bz2", "test_mbdump.tar.bz2")
