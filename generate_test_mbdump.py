...
def extract_keys_and_slice(source_tar, output_tar):
        ...
        with src.extractfile(track_member) as f:
            for _ in range(300):  # Sample 300 tracks
                ...
                if len(parts) > 7:
                    ...
                    recording_ids.add(parts[2])  # recording (id link)
                    medium_ids.add(parts[3])  # medium (id link)

    # Step 2: Read bridge tables to gather dependent entity keys using CORRECT index numbers
                ...
                with src.extractfile(member) as f:
                    for line in f:
                        ...
                        if parts[0] in medium_ids:
                            release_ids.add(parts[1])  # release (id link)
                ...
                with src.extractfile(member) as f:
                    for line in f:
                        ...
                        if parts[0] in release_ids:
                            release_group_ids.add(parts[4])  # release_group is index 4!
                            artist_credit_ids.add(parts[3])  # artist_credit is index 3!

            elif member.name == "mbdump/l_recording_url":
                with src.extractfile(member) as f:
                    for line in f:
                        parts = line.decode('utf-8', errors='ignore').strip('\r\n').split('\t')
                        if len(parts) > 3 and parts[2] in recording_ids:  # entity0 is recording
                ...
                with src.extractfile(member) as f:
                    for line in f:
                        ...
                        if parts[0] in artist_credit_ids:
                            artist_ids.add(parts[2])  # artist ID link is index 2!
        ...
        for member in src:
            if member.name == "mbdump/l_artist_url":
                with src.extractfile(member) as f:
                    for line in f:
                        parts = line.decode('utf-8', errors='ignore').strip('\r\n').split('\t')
                        if len(parts) > 3 and parts[2] in artist_ids:  # entity0 is artist
                            url_ids.add(parts[3])  # entity1 is url
        ...
        for member in src:
            ...
            if name in filter_rules:
                ...
                with src.extractfile(member) as f:
                    for line in f:
                        if idx is None:
                            matched_lines.append(line)  # pass through baseline definitions
                        else:
                            parts = line.decode('utf-8', errors='ignore').strip('\r\n').split('\t')
                            # Ensure the row has enough columns and matches our key map
                            if len(parts) > idx and parts[idx] in lookup_set:
                                # CRITICAL FIX: Ensure the row trailing newline is clean without splitting data records
    ...
    print("\nDone! Relational mini test dataset is completely ready and structurally solid.")


if __name__ == "__main__":
    # Run this to generate a clean test file
