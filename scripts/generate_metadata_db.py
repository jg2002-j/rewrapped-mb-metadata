...
def extract_keys_and_slice(source_tar, output_tar):
    print(f"Analyzing {source_tar} to build an ultra-fast, lightweight test set...")
    ...
    # Step 1: Read tracks first to establish our tiny baseline sample
    print("  -> Phase 1: Rapid sampling of base tracks...")
    with tarfile.open(source_tar, "r:bz2") as src:
        track_member = src.getmember("mbdump/track")
        with src.extractfile(track_member) as f:
            for _ in range(15):  # Dropped down to a micro-sample of 15 tracks for speed
                line = f.readline()
                if not line:
                    break
                ...
                with src.extractfile(member) as f:
                    for line in f:
                        ...
                        if parts[0] in medium_ids:
                            ...
                            if len(release_ids) == len(medium_ids): break # Short-circuit
                ...
                with src.extractfile(member) as f:
                    for line in f:
                        ...
                        if parts[0] in release_ids:
                            ...
                            if len(release_group_ids) == len(release_ids): break # Short-circuit
                ...
                with src.extractfile(member) as f:
                    for line in f:
                        ...
                        if parts[0] in release_group_ids:
                            ...
                            if len(artist_credit_ids) == len(release_group_ids): break # Short-circuit
                    ...
                    for line in f:
                        if idx is None:
                            ...
                            if len(parts) > idx and parts[idx] in lookup_set:
                                matched_lines.append(line)
                                # Since targets are tiny, stop looking once we match everything we need
                                if len(matched_lines) == len(lookup_set) and name not in ["mbdump/l_recording_url", "mbdump/l_artist_url"]:
                                    break
    ...
    print("\nDone! Micro relational test dataset is completely ready.")
